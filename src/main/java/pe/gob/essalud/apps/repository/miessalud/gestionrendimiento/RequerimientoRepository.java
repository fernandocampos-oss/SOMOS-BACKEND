...
public interface RequerimientoRepository extends JpaRepository<Requerimiento, Integer> {
    ...
    @Transactional
    @Modifying
    @Query(value = "UPDATE requerimiento SET nombre=?, descripcion=?, id_tipo_ingreso=?, identificador=?, fecha_modificacion=?, usuario_modificacion=? WHERE id_requerimiento=? ", nativeQuery = true)
